      ...
      if (entry.split('|').length === 1) {
        ...
        if (entry.split('|')[0].trim().replace(/ /g, "_").match(regex)) {
          ...
          function getplace() {
            ...
            if (textInfo[2].trim() === 'locked' || textInfo[2].trim() === 'unlocked' || textInfo[2].trim().length === 0) {
              return ''
            } else {
              return '<span class="metaPlace">' + textInfo[2].trim() + ',<br></span>'
            }
          }

          var place = getplace()

          metadata = $('<div class="paddedBottom" id="singlePadReadMetadata"><div id="timeStampOuter"><span id="timeStamp">TIMESTAMP</br>' + timeStamp.getHours() + ':' + timeStamp.getMinutes() + ':' + timeStamp.getSeconds() + ' / ' + timeStamp.getDate() + '.' + month + '.' + timeStamp.getFullYear().toString().slice(2) + '</span></div><h1>' + place + '<span class="metaDate">' + textInfo[1].trim() + '</span></h1><h1 class="centerText" ><span class="metaTitle">' + textInfo[0].trim() + '</span></h1><h1><span class="metaEvent">' + eventHeader.replace(/_/g, " ").trim() + '</span></h1></div>');
